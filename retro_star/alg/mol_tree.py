...
from retro_star.viz_tree.visualize_tree import basic_viz_dict
from retro_star.viz_tree.mol_tree_to_paroutes_dict import mol_tree_to_paroutes_dict
...
class MolTree:
    ...
    def _add_reaction_and_mol_nodes(self, cost, mols, parent, template, ancestors, templates_idx=None, cost_reference=None):
        assert cost >= 0
        ...
        reaction_node = ReactionNode(parent, cost, template, templates_idx, cost_reference=cost_reference)
        for mol in mols:
    ...
    def expand(self, mol_node, reactant_lists, costs, templates, templates_idx=None, costs_reference=None):
        ...
        for i in range(len(costs)):
            self._add_reaction_and_mol_nodes(costs[i], reactant_lists[i], mol_node, templates[i], ancestors,
                                             templates_idx=templates_idx[i] if templates_idx else None,
                                             cost_reference=costs_reference[i] if costs_reference is not None else None)
    ...
    def viz_tree_with_pic(self, viz_file):
        route_dict = mol_tree_to_paroutes_dict(self.root)
        basic_viz_dict(
            tree=route_dict,
            filename=viz_file,
            draw_mols=True,
            title=f"Visualizing the retrosynthetic route of {self.target_mol}",
        )